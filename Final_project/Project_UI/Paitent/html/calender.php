...
if (isset($_GET['cancel'])) {
  mysqli_query($connect, "update appointment set userStatus='0' where apid = " . $_GET['id'] . "");
  echo "<script>alert('Appointment Canceled Successful');</script>";
}
?>
          ...
          <div class="flex items-center">
            ...
            <div class="hidden md:block">
              <div class="ml-10 flex items-baseline space-x-4">
                ...
                <a href="./calender.php" class="bg-gray-900 text-white rounded-md px-3 py-2 text-sm font-medium" aria-current="page">Calendar</a>
      ...
      <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
        ...
        <table class=" table table-striped table-hover table-bordered border-gray-400">
          <thead>
            <tr>
              <th scope="col">
                <p class="text-lg leading- font-bold text-gray-900 text-center">Dr Name</p>
              </th>
              <th scope="col">
                <p class="text-lg leading- font-bold text-gray-900 text-center">Reservations</p>
              </th>
              <th scope="col">
                <p class="text-lg leading- font-bold text-gray-900 text-center">Reservations Date</p>
              </th>
              <th scope="col">
              ...
              <th scope="col">
                <p class="text-lg leading- font-bold text-gray-900 text-center">Cancel</p>
              </th>
            </tr>
          </thead>
          <tbody class="table-group-divider">
            <?php
            // session_start();
            $all = "SELECT doctors.doctorName as docname,appointment.*  from appointment join doctors on doctors.id=appointment.doctorId where appointmentDate = CURRENT_DATE() and appointment.userId=" . $_SESSION['uid'] . " ORDER BY postingDate DESC";
            $all_query = mysqli_query($connect, $all);
            if (mysqli_num_rows($all_query) > 0) {
              while ($row = mysqli_fetch_assoc($all_query)) { ?>
                <tr>
                  ...
                  <th scope="col">
                    <p class="text-lg leading- font-bold text-gray-900 text-center"><?php echo $row['postingDate']; ?></p>
                  </th>
                  <th scope="col">
                  ...
                  <th scope="col">
                    ...
                    <p class="text-lg leading- font-bold text-gray-900 text-center"><button class=" text-lg inline-flex items-center rounded-md bg-green-50 px-2 py-1 font-medium text-blue-700 ring-1 ring-inset ring-blue-600"><?php if (($row['userStatus'] == 1) && ($row['doctorStatus'] == 1)) {
                                                                                                                                                                                                                                    echo "Active";
                                                                                                                                                                                                                                  }
                  ...
                  <th scope="col">
                    <p class=" text-lg leading- font-bold text-gray-900 text-center">
                      ...
                      <?php } else if (($row['userStatus'] == 1) && ($row['doctorStatus'] == 1)) { ?>
                        <a href="./calender.php?id=<?php echo $row['apid'] ?>&cancel=update" onClick="return confirm('Are You Sure You Want To Delete ?');" class=" text-lg inline-flex items-center rounded-md bg-green-50  px-2 py-1 font-medium text-red-700 ring-1 ring-inset ring-red-600/20"> cancel </a>
                      <?php 
                      
                    } else if (($row['userStatus'] == 0) || ($row['doctorStatus'] == 0)) {
                        echo "<p class=\"  text-lg inline-flex items-center rounded-md bg-green-50  px-2 py-1 font-medium text-red-700 ring-1 ring-inset ring-red-600/20\"> canceled </p>";
                      } ?>
                    </p>
                  </th>
                  </tr>
